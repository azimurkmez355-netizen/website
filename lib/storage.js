...
// On conflict the SDK throws either the specific BlobPreconditionFailedError
// or, in practice, a plain BlobError with the same "conflicting operation"
// meaning — retry on any BlobError from put(), not just the subclass.
async function updateVisits(mutate) {
  const { get, put, BlobError } = require('@vercel/blob');
  ...
  for (let attempt = 0; attempt < MAX_WRITE_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      if (err instanceof BlobError && attempt < MAX_WRITE_ATTEMPTS - 1) {
        // Backoff widens each round so a burst of simultaneous visitors
        // spreads out instead of repeatedly landing on top of each other.
        // Capped low (150ms) and MAX_WRITE_ATTEMPTS kept modest so the
        // whole retry loop can never approach Vercel's function timeout.
        const base = Math.min(25 * (attempt + 1), 150);
